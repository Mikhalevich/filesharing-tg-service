...
RUN CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -a -installsuffix cgo -ldflags="-w -s" -o /go/bin/filesharing-tg-service

FROM alpine:latest

WORKDIR /app/
COPY --from=builder /go/bin/filesharing-tg-service /app/filesharing-tg-service
